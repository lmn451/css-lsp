...
async function testDebugLoggingInProductionMode() {
	...
	assert(
		logCalls.length === 0 || logCalls.every(msg => !msg.includes('[DEBUG]')),
		'Production mode: No debug logs should be written'
	);
...
async function testNoFileWritesInProduction() {
	...
	try {
		...
		if (fs.existsSync(tmpLogPath)) {
			...
			assert(
				age > 5000,
				'Production mode: tmp/css.log should not be recently modified'
			);
		} else {
...
async function testDebugEnvVarCheck() {
	...
	for (const testCase of testCases) {
		...
		assertEquals(
			isDebugEnabled,
			testCase.expected,
			`CSS_LSP_DEBUG=${testCase.desc} → debug=${isDebugEnabled}`
		);
	}

...
// Test 1: Basic Specificity Calculation
{
  ...
  assert.strictEqual(
    rootSpec.classes,
    1,
    ":root should have 1 class (pseudo-class)"
  );
  assert.strictEqual(rootSpec.elements, 0, ":root should have 0 elements");
  console.log(
    "Test 1 passed: :root specificity " + formatSpecificity(rootSpec)
  );
}
...
// Test 3: Class Selector
{
  ...
  console.log(
    "Test 3 passed: .button specificity " + formatSpecificity(classSpec)
  );
}
...
// Test 5: Complex Selector
{
  ...
  console.log(
    "Test 5 passed: div.button#submit specificity " +
      formatSpecificity(complexSpec)
  );
}
...
// Test 6: Specificity Comparison
{
  ...
  assert.strictEqual(
    compareSpecificity(div, root),
    -1,
    "div should be less specific than :root (pseudo-class)"
  );
  assert.strictEqual(
    compareSpecificity(cls, div),
    1,
    ".button should be more specific than div"
  );
  assert.strictEqual(
    compareSpecificity(id, cls),
    1,
    "#main should be more specific than .button"
  );
  assert.strictEqual(
    compareSpecificity(root, root),
    0,
    "Equal specificity should return 0"
  );
...
{
  assert.strictEqual(
    matchesContext(":root", "div"),
    true,
    ":root should match any context"
  );
  assert.strictEqual(
    matchesContext("div", "div"),
    true,
    "Same selector should match"
  );
  assert.strictEqual(
    matchesContext(":root", ".button"),
    true,
    ":root is universal"
  );
